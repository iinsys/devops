...
🚀 DevOps Journey – Day 7  
Task: Configure Password-less SSH from thor (Jump Host) to all App Servers using their respective sudo users.
...
🛠️ One-Block Script to Setup Password-less SSH
```bash
...
# ========= [ 📌 Notes ] =========
# This allows the 'thor' user on Jump Host to SSH into each App Server using the respective sudo user.
# This is critical for automated scripts to perform tasks without needing interactive login.
```